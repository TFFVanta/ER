...
function page(id) {
  ...
  document.querySelectorAll("[data-page]").forEach(el => {
    el.classList.toggle("active", el.dataset.page === id && el.classList.contains("workspace-nav__item"));
  });
...
async function status() {
  try {
    ...
    // Keep the last visible state instead of flashing empty UI on transient failures.
  }
}
...
async function openFile(pathValue) {
  try {
    const data = await api("/api/files?path=" + encodeURIComponent(pathValue));
    const draft = state.drafts.get(pathValue);

    state.openedFile = pathValue;
    fileName.textContent = data.name;
    editor.value = draft ?? data.content ?? "";
